"""
----------------------------------------------------------------------
파일명     : user_routes_complete.py
설명       : 일반 사용자 및 공통 기능을 위한 API 라우터
...
2. 회원가입 및 사용자 관리
   - 회원가입 (기술 스택 포함)
   - 아이디/이메일 중복 확인 및 인증 코드 전송
   - 이메일 인증 처리 (/verify-email)
   - 비밀번호 재설정 (이메일 기반)
...
6. 포트폴리오 테스트
   - 샘플 포트폴리오 전체 목록 조회 (기술 태그 포함)

사용 권한
----------------------------------------------------------------------
- 일반 사용자 전용 기능
- JWT 인증 필요 (일부 기능)
- 일부 기능은 비회원도 접근 가능 (아이디/이메일 중복 체크 등)

비고
----------------------------------------------------------------------
- DB 연결은 pymysql 사용 (DictCursor)
- SQL 실행 후 커넥션 반드시 close
- 이메일 인증은 email_verification 테이블 기반으로 처리
- 인증 코드는 6자리 랜덤 + 3분 만료
- 기술 스택은 공통코드 (TECH_STACK) 기반 분류
- 문의사항 등록 시 관리자에게 알림(alerts) 자동 전송
----------------------------------------------------------------------
"""
