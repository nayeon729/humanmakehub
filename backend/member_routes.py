"""
----------------------------------------------------------------------
파일명     : member_routes.py
설명       : 개발자(Member, R01) 사용자 전용 API 라우터
...
2. 프로젝트 참여 및 초대
   - 참여 프로젝트 조회 (join_requests, team_member 기준)
   - 초대 목록 조회 및 수락/거절 처리
   - 확정된 프로젝트 리스트 반환

3. 프로젝트 채널 (board02: 개별 채널)
   - 채널 게시글 작성 (파일 업로드 포함)
   - 채널 게시글 수정/삭제/상세 조회
   - 특정 프로젝트 팀원과의 채널 게시글 목록 조회 (페이징)
   - 프로젝트의 전체 공지사항 조회 (board01)

4. 기술 스택 분류 조회
   - 공통 코드 기반의 기술 스택 카테고리 및 기술명 제공

사용 권한
----------------------------------------------------------------------
- 개발자(Member, R01)만 접근 가능
- JWT 인증을 통해 사용자 검증
- 일부 기능은 관리자(R02)/PM(R03) 접근 허용 (게시글 등)

비고
----------------------------------------------------------------------
- DB 연결은 pymysql 사용 (DictCursor)
- SQL 실행 후 커넥션 반드시 close
- 파일 업로드 경로는 /uploads/projectchannel 하드코딩됨
- 일부 요청은 FormData 및 UploadFile 병행 처리
- 알림(alerts) 등록 시 FRONT_BASE_URL 이용
----------------------------------------------------------------------
"""
